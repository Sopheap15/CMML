...
r_da <- r_data %>%
  ...
  summarise(instock = round_half_up(sum(remaining_stock_item_batch, na.rm = T))) %>% 
...
Sheep blood is a vital supplement to support bacteria growth and visualize hemolysis patterns. To sustain sheep blood supply for CMML, DMDP established an agreement with Resource Development International Cambodia (RDIC) and EUROVET veterinary clinics for care, feeding, bleeding and housing of the sheep. Detail of [sheep inventory](https://docs.google.com/spreadsheets/d/1ixEijAjGdYoIG56HROUVuN9z_DXIak1A6achRbUAzFM/edit#gid=1304838431) 
...
We compute media production data from [1 Jan - `r format(max(data$preparation_date),"%d %b %Y")`]{style="color:red"}
...
```{r production variation, results = 'asis'}
p_data <- data %>% 
    filter(preparation_date >= "2021-01-01") %>% 
...
Media distribution is calculated from [1 Jan - `r format(max(d_data$delivery_date),"%d %b %Y")`]{style="color:red"}

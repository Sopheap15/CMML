---
metadata title: "<center> Central Media Making Laboratory <br> Report </center>"
output:
  html_document:
    theme: cerulean
---
...
<br>
\newpage
Dehydrated media powder is critical for the production of culture media. Based on media production data, we express dehydrated media in grams to determine the consumption.
...
p_data %>% 
  ...
  scale_y_continuous(expand = c(0.01,0.01), 
                     limits = c(0,max(p_data$total) + 1500), 
                     label = scales::comma) +
...
Frequency of sheep blood collection from [`r format(date[1,2], "%d %b")` - `r format(Sys.Date(),"%d %b %Y")`]{style="color:red"}:
...
Date: [`r format(date[1,2], "%d %b")` - `r format(Sys.Date(),"%d %b %Y")`]{style="color:red"}   
...
```{r monthly distribution, fig.width= 9}
d_data <- data %>%
	filter(delivery_date >= date[3, 2], delivery_date < date[4, 2]) %>% 
...
```{r media distribute by customer type and media type, fig.width = 10}
d_data <- data %>% 
	filter(delivery_date >= date[3, 2], delivery_date < date[4, 2]) %>% 
...
geom_bar(stat = "identity") +
	...
	scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(.01, 0.10))) +
	theme_bw() +
	theme(legend.position = "non",
